...
def filteringSNPS(variants, cadd_sort, samples, G,
    ...
    for var in range(len(variants) // interval):
        ...
        row = row.values
        row_, variants_, cadd_ = ld_prune(row, variants[ind:ind + interval], cadd_sort[ind:ind + interval],
...
data_d, data_s, variants_, cadd_ = filteringSNPS(variants, cadd_sort.CADD_PHRED.values, samples, G, path_input,
                                                 path_output, 'first')

'''FINAL PRUNE: IF 0 IN ONE AND 1 ON ANOTHER: 2, IF 1 AND 0: 0, IF 0 AND 0: 1'''
data_df, data_sf, variants_f, cadd_f = filteringSNPS(np.array(variants_), np.array(cadd_), samples, G, path_input,
                                                     path_output, 'final', 0.2, 10000)
